...
public class HLPReporteControlObs implements Serializable {
    ...
    public HLPReporteControlObs(FormRepControlObs formFiltros) {
        switch (formFiltros.getFormatoDeTabla()) {
            ...
            case 14:
                ...
                break;
            case 15:
                crearFormatoDeTablaQuince(formFiltros);
                break;
            default:
                break;
        }
    ...
    private void crearFormatoDeTablaUno(FormRepControlObs formFiltros) {
        ...
        //Pie de Página
        hlpEncabezado = new HLPReportesEncabezado();
        hlpEncabezado.ingresarEncabezado(0, columnas, 1, "* Válidas es el resultado de las solicitudes aprobadas menos solicitudes canceladas ", 3);
        listaEncabezados.add(hlpEncabezado);
    }
    ...
    private void crearFormatoDeTablaDos(FormRepControlObs formFiltros) {
        ...
        hlpEncabezado.ingresarEncabezado(4, 3, 1, "Pendientes", 1);
        hlpEncabezado.ingresarEncabezado(5, 3, 1, "Aprobadas", 1);
        hlpEncabezado.ingresarEncabezado(6, 3, 1, "Denegadas / Canceladas", 1);
        hlpEncabezado.ingresarEncabezado(7, 3, 1, "*Válidas", 1);
        listaEncabezados.add(hlpEncabezado);
        //Tercer encabezado
        hlpEncabezado = new HLPReportesEncabezado();
        hlpEncabezado.ingresarEncabezado(0, 1, 1, "Cons. Loc", 1);
        hlpEncabezado.ingresarEncabezado(1, 1, 1, "Cons. Dttal", 1);
        hlpEncabezado.ingresarEncabezado(2, 1, 1, "Cons. Loc", 1);
        hlpEncabezado.ingresarEncabezado(3, 1, 1, "Cons. Dttal", 1);
        hlpEncabezado.ingresarEncabezado(4, 1, 1, "Cons. Loc", 1);
        hlpEncabezado.ingresarEncabezado(5, 1, 1, "Cons. Dttal", 1);
        hlpEncabezado.ingresarEncabezado(6, 1, 1, "Cons. Loc", 1);
        hlpEncabezado.ingresarEncabezado(7, 1, 1, "Cons. Dttal", 1);
        hlpEncabezado.ingresarEncabezado(8, 1, 1, "Total", 1);
        hlpEncabezado.ingresarEncabezado(9, 1, 1, "Cons. Loc", 1);
        hlpEncabezado.ingresarEncabezado(10, 1, 1, "Cons. Dttal", 1);
        hlpEncabezado.ingresarEncabezado(11, 1, 1, "Total", 1);
        hlpEncabezado.ingresarEncabezado(12, 1, 1, "Cons. Loc", 1);
        hlpEncabezado.ingresarEncabezado(13, 1, 1, "Cons. Dttal", 1);
        hlpEncabezado.ingresarEncabezado(14, 1, 1, "Total", 1);
        hlpEncabezado.ingresarEncabezado(15, 1, 1, "Cons. Loc", 1);
        hlpEncabezado.ingresarEncabezado(16, 1, 1, "Cons. Dttal", 1);
        hlpEncabezado.ingresarEncabezado(17, 1, 1, "Total", 1);
        listaEncabezados.add(hlpEncabezado);
    ...
    private void crearFormatoDeTablaCuatro(FormRepControlObs formFiltros) {
        ...
        hlpEncabezado.ingresarEncabezado(6, 1, 2, "Denegadas", 1);
        hlpEncabezado.ingresarEncabezado(7, 1, 2, "Canceladas", 1);
        hlpEncabezado.ingresarEncabezado(8, 1, 2, "Declinadas", 1);
    ...
    private void crearFormatoDeTablaCinco(FormRepControlObs formFiltros) {
        ...
        hlpEncabezado.ingresarEncabezado(9, 1, 2, "Declinadas", 1);
        hlpEncabezado.ingresarEncabezado(10, 1, 2, "*Válidas", 1);
        listaEncabezados.add(hlpEncabezado);
    ...
    private void crearFormatoDeTablaSeis(FormRepControlObs formFiltros) {
        ...
        hlpEncabezado.ingresarEncabezado(6, 1, 2, "Declinadas", 1);
    ...
    private void crearFormatoDeTablaQuince(FormRepControlObs formFiltros) {
        columnas = 1;
        listaEncabezados = new ArrayList<>();
        int index;
        String filtro;
        HLPReportesEncabezado hlpEncabezado = new HLPReportesEncabezado();
        //Primer encabezado
        hlpEncabezado.ingresarEncabezado(0, 1, 2, "Instancia", 1);
        index = 1;
        if (formFiltros.getEncabezadoCanDenDecl() != null) {
            for (int[] resultado : formFiltros.getEncabezadoCanDenDecl()) {
                switch (resultado[0]) {
                    case 2:
                        filtro = "Denegadas";
                        break;
                ...
                hlpEncabezado.ingresarEncabezado(index, resultado[1] + 1, 1, filtro + "", 1);
                columnas = columnas + resultado[1] + 1;
                index++;
            }
        }
        if (formFiltros.getEncabezadoCanDenDecl().size() > 1) {
            hlpEncabezado.ingresarEncabezado(index, 1, 2, "TOTAL", 1);
            columnas = columnas + 1;
        }
        listaEncabezados.add(hlpEncabezado);
        ...
        if (formFiltros.getEncabezadoCanDenDecl() != null) {
            for (int[] resultado : formFiltros.getEncabezadoCanDenDecl()) {
                for (int i = 0; i < resultado[1]; i++) {
                    hlpEncabezado.ingresarEncabezado(index, 1, 1, columna + "", 1);
                    columna++;
                    index++;
                }
                hlpEncabezado.ingresarEncabezado(index, 1, 1, "Total", 1);
                index++;
            }
        }
        listaEncabezados.add(hlpEncabezado);
        ...
        //Pie de Página
        columna = 1;
        if (formFiltros.getDescripcionJust() != null) {
            for (String descripcion : formFiltros.getDescripcionJust()) {
                    hlpEncabezado = new HLPReportesEncabezado();
                    hlpEncabezado.ingresarEncabezado(0, columnas, 1, columna+".- "+descripcion , 3);
                    listaEncabezados.add(hlpEncabezado);
                    columna++;
                }
            }
