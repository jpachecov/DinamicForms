...
public class DTOReportesParametros implements Serializable {
    ...
    private boolean muestraTotales;
    ...
    public HSSFWorkbook crearEncabezadoXLS(HSSFWorkbook libro) {
		...
		if (muestraTotales) {
			numRow++;
			HSSFRow infoTotales = hoja.createRow(numRow);
			infoTotales.setHeight((short) 300); // alto de fila
			HSSFCell totales = infoTotales.createCell(0);
			totales.setCellValue("Total: " + getListaDatos().size());
			CellRangeAddress rangeTotales = new CellRangeAddress(numRow, numRow, 0, numColumnas);
			hoja.addMergedRegion(rangeTotales);
			totales.setCellStyle(estiloInfo);
		}
...
//        Encabezados de la tabla
    ...
    /**
     * Método que obtiene el valor de el atributo muestraTotales
     * @return
     */
	...
	/**
	 * Método que ingresa el valor de el atributo muestraTotales
	 * @param muestraTotales
	 */
