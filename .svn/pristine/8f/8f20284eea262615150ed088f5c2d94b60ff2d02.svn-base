 ...
 * CSRF token. It is called for all requests to backend destination.
 *
 * We use the approach in which: - The CSRF token is changed after each valid
 * HTTP exchange - The custom Header name for the CSRF token transmission is
 * fixed - A CSRF token is associated to a backend service URI in order to
 * enable the support for multiple parallel Ajax request from the same
 * application - The CSRF cookie name is the backend service name prefixed with
 * a fixed prefix
 *
...
public class CSRFValidationFilter implements Filter {
	...
	/**
	 * Arreglo con todas las URL desde donde es permitido hacer peticiones POST
	 * al sistema de Observadores.
	 */
	private final String[] trustedOrigins = { "https://localhost:8443/observadoresINE/app",
			"https://pef2015pruebas.ine.mx/observadoresINE/app", "https://10.0.28.88:8443/observadoresINE/app",
			"https://10.0.28.87:8443/observadoresINE/app", "https://10.0.28.110:8443/observadoresINE/app",
			"https://10.0.28.111:8443/observadoresINE/app", "https://10.0.41.243:8443/observadoresINE/app",
			"https://10.0.41.244:8443/observadoresINE/app", "https://10.0.28.114:8443/observadoresINE/app",
			"https://10.0.28.115:8443/observadoresINE/app", "https://prue-pef2015-seg.ine.mx/observadoresINE/app/",
			"https://capajb6.ine.mx/observadoresINE/app", "https://observadores.ine.mx/observadoresINE/app" };

	/**
	 * Name of the custom HTTP header used to transmit the CSRF token and also
	 * to prefix the CSRF cookie for the expected backend service
	 */
	...
	private String TOKEN_SEPARADOR = "8443";
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if (httpReq.getMethod().equals("POST")) {
			...
			LOG.info("Origin: " + source);
			asignaOrigenConfiable(source);
			...
			if (esConfiable) {
				...
				if (!this.targetOrigin.getProtocol().equals(sourceURL.getProtocol())
					...
					// block the request
					accessDeniedReason = String.format(
							"CSRFValidationFilter: Protocol/Host/Port do not fully matches so we block the request! (%s != %s) ",
				...
				accessDeniedReason = String.format(
						"CSRFValidationFilter: No se puoo asignar una URL de origen confiable en este ambiente.");
				LOG.warn(accessDeniedReason);
				httpResp.sendError(HttpServletResponse.SC_FORBIDDEN, accessDeniedReason);
			}

			/*
			 * STEP 2: Verifying CSRF token using "Double Submit Cookie"
			 * approach
			 */
			// If CSRF token cookie is absent from the request then we provide
			// one in response but we stop the process at this stage.
			// Using this way we implement the first providing of token
			// Cookie tokenCookie = null;
			// if (httpReq.getCookies() != null) {
			// String csrfCookieExpectedName =
			// this.determineCookieName(httpReq);
			// //tokenCookie = Arrays.stream(httpReq.getCookies()).filter(c ->
			// c.getName().equals(csrfCookieExpectedName)).findFirst().orElse(null);
			// for(Cookie c: httpReq.getCookies()){
			// if(c.getName().equals(csrfCookieExpectedName)){
			// tokenCookie = c;
			// break;
			// }
			// }
			...
			// {
			// LOG.info("CSRFValidationFilter: CSRF cookie absent or value is
			// null/empty so we provide one and return an HTTP NO_CONTENT
			// response !");
			// //Add the CSRF token cookie and header
			// this.addTokenCookieAndHeader(httpReq, httpResp);
			// //Set response state to "204 No Content" in order to allow the
			// requester to clearly identify an initial response providing the
			// initial CSRF token
			// httpResp.setStatus(HttpServletResponse.SC_NO_CONTENT);
			// } else {
			// //If the cookie is present then we pass to validation phase
			// //Get token from the custom HTTP header (part under control of
			// the requester)
			...
			// accessDeniedReason = "CSRFValidationFilter: Token provided via
			// HTTP Header is absent/empty so we block the request !";
			// LOG.warn(accessDeniedReason);
			...
			// //Here is not the case so we trace the event and we block the
			// request
			// accessDeniedReason = "CSRFValidationFilter: Token provided via
			// HTTP Header and via Cookie are not equals so we block the request
			// !";
			// LOG.warn(accessDeniedReason);
			// httpResp.sendError(HttpServletResponse.SC_FORBIDDEN,
			// accessDeniedReason);
			...
			// //Here is the case so we let the request reach the target
			// component (ServiceServlet, jsp...) and add a new token when we
			// get back the bucket
			// HttpServletResponseWrapper httpRespWrapper = new
	 ...
	 * 
	 */
	public void asignaOrigenConfiable(String source) {
		try {
			LOG.info("asignaOrigenConf : source : " + source);
			String[] splits;
			String comp = "";
			splits = source.split("/");
			...
			for (String url : this.trustedOrigins) {
				if (url.contains(comp)) {
					this.targetOrigin = new URL(url);
					LOG.info("Origen confiable: " + url);
					esConfiable = true;
					return;
				}
			}
			esConfiable = false;
			this.targetOrigin = new URL("");
		} catch (Exception e) {
			LOG.error("Error en asignar URL de origen confiable");
			e.printStackTrace();
		}
	 ...
	 *            Source HTTP request
	 * @param httpResponse
	 *            HTTP response object to update
	 */
	private void addTokenCookieAndHeader(HttpServletRequest httpRequest, HttpServletResponse httpResponse) {
		...
		// We let the adding of the "Secure" cookie attribute to the reverse
		// proxy rewriting...
		// Here we lock the cookie from JS access and we use the SameSite new
